pub fn part_a(input: &str) -> i32 {
    ...
    let len = lines[0].len();
    let gamma: i32 = (0..len).map(|i| {
        let count = lines.iter().fold(0, |acc, line| {
...
fn find_rating(mut lines: Vec<&str>, use_most_common: bool) -> i32 {
    for i in 0..lines[0].len() {
        ...
        let target_bit = if (count >= 0) == use_most_common {b'1'} else {b'0'};
        lines = lines.iter().filter(|line| line.as_bytes()[i] == target_bit).cloned().collect();
        if lines.len() == 1 {
